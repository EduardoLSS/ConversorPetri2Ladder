...
def main(page: Page):
        ...
        if e.files:
            try:
                with open(e.files[0].path, 'r') as file:
                    ...
                    destination_file_path = f"{diretorio}\\ladderfile{now}.xml"
    ...
    def on_convert_click(e):
        ...
        if xml_content.value:
            try:
                result = subprocess.run(["python", "petri2ladder_cli.py", "--destination", destination_file_path, "--source",
                                origin_file_path], check=True, capture_output=True, text=True)
                print(result.stdout)
